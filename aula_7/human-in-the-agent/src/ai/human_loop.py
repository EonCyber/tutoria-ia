...
class AssistantHumanLoop:
    ...
    async def async_init(self):
        ...
        self.llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.5).bind_tools(self.tools)
    ...
    def make_assistant_node(self, system_prompt):
        def assistant_node(state: AssistantState):
            ...
            return { "messages": [self.llm.invoke(final_prompt)] }
    ...
    def make_editor_node(self, system_prompt):
        def editor_node(state: AssistantState):
            ...
            return { "messages": [self.llm.invoke(final_prompt)], "human_feedback": True }
        return editor_node
    ...
    def make_generate_contract_node(self):
        ...
        def generate_contract_node(state: AssistantState):
            ...
            contract_generated = self.llm.invoke(final_prompt).content
            ...
            final_message = self.llm.invoke(final_prompt_ask).content
    ...
    def make_save_contract_condition(self):
        ...
        def save_contract_condition(state: AssistantState):
            print("Invocando Condition de Salvamento")
            messages = state['messages'][-3:]
            final_prompt = ChatPromptTemplate.from_messages([("system",""" 
    ...
    async def build_graph(self):
        ...
        builder.add_conditional_edges('assistant', conditional_node, { "need_tool": "tools","has_contract_info": "generate_contract_node", "end": END })
        builder.add_edge('tools', 'assistant')
        builder.add_edge('generate_contract_node', 'human_approval_node')
        builder.add_edge('human_approval_node', 'contract_editor')
        builder.add_conditional_edges('contract_editor', save_contract_condition, { "approved": 'save_contract_node', "edit_ready": 'generate_contract_node', 'need_info': 'human_approval_node'})
